...
function restful( method, path, content, callback ) {
	...
	if (err)
	    ...
	    winston.debug( "REST: " + method + " " + path );
...
module.exports.cookie = function( callback ) {
    restful( 'GET', '/users/me', '',
	     function(err, res) {
		 if (err)
		     callback( err, undefined );
		 else
		     callback( null, res.headers['set-cookie'][0] );
	     });
};
...
function githubOwnerAndRepo( repositoryPath, callback ) {
	...
	git.Remote.list(repo).then(function(array) {
		...
		git.Remote.lookup(repo, name).then(function(remote) {
		    ...
		    // The .git is apparently optional, but there cannot be dots in the repository name
		    // BADBAD: shouldn't I just eb able to make \.git into something non-greedy
...
module.exports.publishCommit = function( repositoryPath, callback ) {
	...
	if (err)
		...
		[
		    function(callback) {
			winston.debug( "Getting owner and repo for " + repositoryPath );			
			...
			winston.debug( "Publishing to " + publicationLocation );
			callback(null);
		    }
		],
		function(err) {
		    if (err) {
			winston.debug( "Publication error: " + err );
...
var publishBinaryFile = function( url, filename, callback ) {
    fs.readFile( filename, function(err, data) {
	if (err)
	    callback(err);
	else {
	    restful( 'PUT', url, Buffer.from(data), callback );
	}
    });
};

var publishHtmlFile = function( url, filename, callback ) {
...
var publishFile = function( kind, repositoryPath, filename, callback ) {
	...
	if (err)
		...
		publishHtmlFile( url, filename, callback );
	    } else {
		publishBinaryFile( url, filename, callback );
	    }
	}
    });
};
